...
public class ClientSingleton {
    public static ClientSingleton Instance { get; } = new();
    private readonly string JSONRPC = "2.0";
    private const string LOCAL_HOST = @"http://localhost";
    private const string TOKEN = "bvdownkr";
    ...
    /// <returns></returns> 
    public async Task<AriaAddUri?> AddUriAsync(List<string> uris, AriaSendOption option, int position = -1)
    {
    ...
    /// <returns></returns>
    public async Task<AriaAddTorrent?> AddTorrentAsync(string torrent, List<string> uris, AriaSendOption option, int position = -1)
    {
        List<object> ariaParams =
        [
            "token:" + token,
    ...
    /// <param name="option"></param>
    /// <returns></returns>
    public async Task<AriaAddMetalink?> AddMetalinkAsync(string metalink, List<string> uris, AriaSendOption option, int position = -1)
    {
        List<object> ariaParams =
        [
            "token:" + token,
    ...
    /// <returns></returns>
    public async Task<AriaChangeUri?> ChangeUriAsync(string gid, int fileIndex, List<string> delUris, List<string> addUris, int position = -1)
    {
        List<object> ariaParams =
        [
            "token:" + token,
    ...
    /// <returns></returns>
    public async Task<List<SystemMulticall>?> MulticallAsync(List<SystemMulticallMathod> systemMulticallMathods)
    {
        List<object> ariaParams =
        [
    ...
    public void SetToken(string token = TOKEN) {
        this.token = token;
    }
    ...
    public void SetHost(string host = LOCAL_HOST) {
        this.host = host;
    }

    public void SetListenPort(int listenPort = LISTEN_PORT) {
    ...
    /// </summary>
    /// <returns></returns>
    private string GetRpcUri() {
        return string.Format("{0}:{1}/jsonrpc", host, listenPort);
    }
    ...
    {
        // 需要去掉null,添加 相应的 options
        JsonSerializerOptions options = new() {
            DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingDefault,
            WriteIndented = true,
        };
        // 转换为json字符串
        string sendJson = JsonUtils.SerializeJsonObj(ariaSend, options);
        // 向服务器请求数据
        ...
        // 反序列化
        var aria = JsonUtils.ParseJsonString<T>(result);
        return aria;
    }
    ...
    private static string? Request(string url, string parameters, int retry = 3)
    {
        ...
        try
        {
            ...
            ms.Position = 0;
            requestMessage.Content = new StreamContent(ms, (int)ms.Length);
            ...
            CoreManager.logger.Error("AriaClient", e);
